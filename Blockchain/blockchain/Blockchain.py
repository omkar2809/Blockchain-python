...
class Blockchain:
    ...
    def submit_transaction(self, sender_public_key, recepient_public_key, signature, amount):
        transaction = OrderedDict({
            'sender_public_key':sender_public_key,
            'recepient_public_key': recepient_public_key,
            'amount': amount
        })
    ...
    def valid_chain(self, chain):
        ...
        while current_index < len(chain):
            ...
            if block['previous_hash'] != self.hash(last_block):
                return False
            transactions = block['transactions'][:-1]   # ignore reward transaction
            transaction_elements = ['sender_public_key', 'recepient_public_key', 'amount']
...
def mine():
    ...
    blockchain.submit_transaction(sender_public_key = MINING_SENDER,
                                  recepient_public_key=blockchain.node_id,
                                  signature='',
...
def new_transaction():
    ...
    required = ['confirmation_sender_public_key',
                'confirmation_recepient_public_key',
                'signature',
                'confirmation_amount']
    ...
    transaction_results = blockchain.submit_transaction(values['confirmation_sender_public_key'],
                                                        values['confirmation_recepient_public_key'],
                                                        values['signature'],
